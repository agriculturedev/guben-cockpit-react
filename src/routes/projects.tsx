...
function ProjectsComponent() {
    const {pageSize, page, pageCount, total, previousPage, setPageIndex, nextPage} = usePagination();
  ...
  return (
      <>
        <View title={projectViewData?.data?.attributes?.Title} description={projectViewData?.data?.attributes?.Description} isLoading={projectViewIsLoading}>
          <PaginationContainer page={page} pageSize={pageSize} pageCount={pageCount} total={total} nextPage={nextPage} previousPage={previousPage} setPageIndex={setPageIndex} />
        </View>
      </>
  );
}
